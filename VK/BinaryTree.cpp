...
void run( std::istream & input, std::ostream & output ) {
    ...
    IntCmp cmp;
    BinaryTree<int, IntCmp> Tree( cmp ); // Инициализация дерева

    int n;
    input >> n;

    for ( int i = 0, value; i < n && input >> value; i++ ) Tree.Add( value );

    Tree.Traverse( [] ( const int & value ) { std::cout << value << " "; }); // Выводим элементы в порядке post-order
